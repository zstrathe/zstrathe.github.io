################################################
####         Updated by Zack Strathe        ####
####           for CIS 732 Project          ####
####       to collect training losses       ####
####      of GAN models to a .csv file      ####
################################################
...
class Trainer(AbstractTrainer):
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, eval=True):
        ...
        print('generate corpus', generate_corpus)
        print('ref corpus', reference_corpus)
...
class GANTrainer(Trainer):
    ...
    def __init__(self, config, model):
        ...
        self.training_loss = {'Generator_pretraining':[], 
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True):
        #self.resume_checkpoint('saved/SeqGAN-lyrics-Apr-17-2022_15-01-42.pth')
        ...
        for epoch_idx in range(self.g_pretraining_epochs):
            ...
            if verbose:
                self.logger.info(train_loss_output)
            self.training_loss['Generator_pretraining'].append({'epoch': epoch_idx, 'loss': train_loss}) #####
        if verbose:
            self.logger.info("End generator pretraining...")
        with open('saved/' + self.config['filename'] +'-GeneratorPretrainingLog.csv', 'w') as csvfile:  ####
            field_names = ['epoch', 'loss']
            writer = csv.DictWriter(csvfile, fieldnames=field_names)
            writer.writeheader()
            writer.writerows(self.training_loss['Generator_pretraining'])
        ...
        for epoch_idx in range(self.d_pretraining_epochs):
            ...
            if verbose:
                self.logger.info(train_loss_output)
            self.training_loss['Discriminator_pretraining'].append({'epoch': epoch_idx, 'loss': train_loss})
        if verbose:
            self.logger.info("End discriminator pretraining...")
        with open('saved/' + self.config['filename'] +'-DiscriminatorPretrainingLog.csv', 'w') as csvfile:
            field_names = ['epoch', 'loss']
            writer = csv.DictWriter(csvfile, fieldnames=field_names)
            writer.writeheader()
            writer.writerows(self.training_loss['Discriminator_pretraining'])
        ...
        for epoch_idx in range(self.adversarail_training_epochs):
            ...
            if verbose:
                self.logger.info(train_loss_output)
            self.training_loss['Adversarial_training'].append({'epoch': epoch_idx, 'loss': train_loss})
            ensure_dir('saved/plots/')
            #self.plot_train_loss(show=True, save_path=os.path.join('saved/plots/','adversarial_epoch' + str(epoch_idx)))

        if verbose:
            self.logger.info("End adversarial pretraining...")
        with open('saved/' + self.config['filename'] +'-AdversarialTrainingLog.csv', 'w') as csvfile:
            field_names = ['epoch', 'loss']
            writer = csv.DictWriter(csvfile, fieldnames=field_names)
            writer.writeheader()
            writer.writerows(self.training_loss['Adversarial_training'])
...
class LeakGANTrainer(GANTrainer):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True):
        ...
        if verbose:
            self.logger.info(">> Start adversarial training")
        for epoch in range(int(self.iters_num)):
            if verbose:
            ...
            for epoch_idx in range(self.adversarail_training_epochs):
                ...
                self.training_loss['Adversarial_training'].append({'epoch': epoch, 'interleaved_epoch': epoch_idx, 'loss': train_loss}) #####
        ...
        with open('saved/' + self.config['filename'] +'-AdversarialTraining.csv', 'w') as csvfile:  ####
            field_names = ['epoch', 'interleaved_epoch', 'loss']
            writer = csv.DictWriter(csvfile, fieldnames=field_names)
            writer.writeheader()
            writer.writerows(self.training_loss['Adversarial_training'])
        self._save_checkpoint(self.adversarail_training_epochs)
